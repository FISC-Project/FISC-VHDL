...
ENTITY UART_Link IS
	PORT(
		...
		fmem_data_read    : in  std_logic_vector(256*8-1 downto 0); -- Read
...
ARCHITECTURE RTL OF UART_Link IS
	...
	-- Flash Memory Control variables:
	...
	signal   fmem_save_data               : std_logic_vector(256*8-1 downto 0) := (others => '0'); -- Save Flash Memory Data
				...
				case state is
					...
					when s_listen => 
						...
						if uart_read_irq = '1' then
							...
							-- TODO: Execute special command when asked, which is the loading of the Boot code from Flash Memory into the SDRAM. This command will be requested by the top entity
					...
					when s_sdram_write =>
						-- Write word to SDRAM:
						iob_sdram_cmd_address <= data_buffer(22 downto 0); --data_buffer(2)(6 downto 0) & data_buffer(1) & data_buffer(0);
